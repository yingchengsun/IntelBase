...
class SaveToFilePipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item, Tweet):
            savePath = os.path.join(self.saveTweetPath, item['query']+'.csv')
            self.save_to_file(item,savePath)
            logger.debug("Add tweet:%s" %item['url'])
        elif isinstance(item, User):
            savePath = os.path.join(self.saveUserPath, 'users.csv')
            self.save_to_file(item, savePath)
            logger.debug("Add user:%s" %item['screen_name'])
    ...
    def save_to_file(self, item, fname):
        ...
        with open(fname,'ab') as f:
            ...
            if self.flag:
                w.writerow(sorted_item.keys())
                self.flag = False
            w.writerow(sorted_item.values()) 
            
            
        '''
        with open(fname, 'a') as f:  # Just use 'w' mode in 3.x
